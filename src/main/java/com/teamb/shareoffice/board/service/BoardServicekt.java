...
public class BoardServicekt {
	...
	public ArrayList<HashMap<String, Object>> getFreeBoardList(int pageNum, String searchOption, String searchStr ){
		
		ArrayList<HashMap<String, Object>> dataList = new ArrayList<HashMap<String, Object>>(); 
		pageNum = ((pageNum - 1) * 10);
		
		System.out.println("서비스 도착");
		ArrayList<FreeBoardVo> freeBoardList = boardMapperkt.getFreeBoardList(searchOption, searchStr, pageNum);
		...
		for(FreeBoardVo freeBoardVo : freeBoardList) {
			...
			String countComment = boardMapperkt.getFreeBoardCommentCount(freeboard_no);
			HashMap<String, Object> addList = new HashMap<String, Object>();

			
			if(countComment != null) {
				addList.put("countComment", countComment);
			}else if(countComment == null) {
				countComment = "0";	
				addList.put("countComment", countComment);
			}
			...
			Date writeDate = freeBoardVo.getFreeboard_writeDate();
			long writeTime = writeDate.getTime();
			
			long currentTime = System.currentTimeMillis();
			long targetTime = currentTime - 1000*60*3;// ms - s*m*h* (3min)
			
			if(writeTime > targetTime) {
				addList.put("newKeyword", true);
			}
	...
	public int getFreeBoardCount(String searchOption, String searchStr) {

		return boardMapperkt.getFreeBoardCount(searchOption, searchStr);
	}
